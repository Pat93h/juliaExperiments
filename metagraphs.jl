...
function setupAgentNetwork(agentcount::Integer)
    ...
    for (index,v) in enumerate(vertices(network))
        set_prop!(network,v, :newNeighbors, newNeighbors)
...
function simulation_step(network::MetaDiGraph)
    ...
    for v in vertices(network)
        ...
        for i in 1:newconnections
            ...
            newEdge = Edge(v,newcandidates[newdst])
            # get

            newNeighborArray = get_prop(network, newneighbor, :newNeighbors)


            append!(newNeighborArray,v)
            println("New neighbor array is $newNeighborArray. Current NewNeighborslist of $v: " * string(get_prop(network, v, :newNeighbors)))

            set_prop!(network, newcandidates[newdst], :newNeighbors, newNeighborArray)

            add_edge!(network,newEdge)
            set_prop!(network, newEdge, :weight, rand(1:20))
            # newneighbor = newcandidates[newdst]
